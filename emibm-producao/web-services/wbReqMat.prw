...
WSMETHOD GET WSRECEIVE CODPRODUTO WSSERVICE PRODUTO
	...
	If (cNextAlias)->(!Eof())
		While (cNextAlias)->(!Eof())
			...
			oProduto:SetDesc(AllTrim((cNextAlias)->B1_DESC))
...
WSMETHOD POST WSRECEIVE PRODUTO WSSERVICE PRODUTO
	...
	If !(SB1->( DbSeek( xFilial("SB1") + oParseJSON:PRODUTO:COD ) ))
		...
		Aadd(aDadosProd, {"B1_DESC", oParseJSON:PRODUTO:DESC, Nil} )
...
WSMETHOD GET WSSERVICE CCUSTO
	...
	If (cNextAlias)->(!Eof())
		While (cNextAlias)->(!Eof())
			...
			oCentroC:SetDesc01(AllTrim((cNextAlias)->CTT_DESC01))
...
WSMETHOD GET WSSERVICE ARMAZEM
	...
	If (cNextAlias)->(!Eof())
		While (cNextAlias)->(!Eof())
			...
			oArma:SetDesc(AllTrim((cNextAlias)->NNR_DESCRI))
			oResponse:Add(oArma)
			oArma := Arma():New()
			(cNextAlias)->(DbSkip())
		EndDo
...
WSMETHOD GET WSSERVICE UNIMED
	...
	If (cNextAlias)->(!Eof())
		While (cNextAlias)->(!Eof())
			...
			oUM:SetUmRes(AllTrim((cNextAlias)->AH_UMRES))
			oResponse:Add(oUM)
			oUM := UM():New()
			(cNextAlias)->(DbSkip())
		EndDo
...
WSMETHOD GET WSSERVICE TIPOPROD
	...
	If (cNextAlias)->(!Eof())
		While (cNextAlias)->(!Eof())
			...
			oTipo:SetDesc(AllTrim((cNextAlias)->X5_DESCRI))
			oResponse:Add(oTipo)
			oTipo := TIPO():New()
			(cNextAlias)->(DbSkip())
		EndDo
...
WSMETHOD POST WSRECEIVE OBJETO WSSERVICE SOLARMA
	...
	For nI := 1 to len(oParseJSON:OBJETO:ITENS)
		...
		Aadd(aArray[Len(aArray)],{"CP_OBS", oParseJSON:OBJETO:OBS, Nil})
	Next
